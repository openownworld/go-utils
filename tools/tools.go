...
func WriteLogError(v ...interface{}) error {
	fileSize := GetFileSizeBySeek(appErrorLogFile)
	...
	log := timeString + " - " + getUpperFuncName() + " - " + fmt.Sprintln(v...)
	fmt.Println(log)
...
func WriteLogErrorf(format string, a ...interface{}) error {
	...
	log := timeString + " - " + getUpperFuncName() + " - " + fmt.Sprintf(format, a...)
	fmt.Println(log)
	file.WriteString(log)
	file.Close()
	return nil
...
//原文：https://blog.csdn.net/xtxy/article/details/51837400
