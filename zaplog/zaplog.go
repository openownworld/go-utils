...
import (
	"errors"
	"fmt"
	"github.com/openownworld/go-utils/iniutils"
...
func init() {
	...
	InitLoggerFile(tools.GetAbsolutePath(logConfigFile))
}

func initConfig(filePath string, v interface{}) error {
	//读配置文件
	iniParser := iniutils.IniParser{}
	//var iniParser iniutils.IniParser
	//filePath := "E:/code-workspace/AppGateway/bin/config.ini"
	if err := iniParser.Load(filePath); err != nil {
		tools.WriteLogErrorf("try load config file %s  ,error %s \n", filePath, err.Error())
		if err := iniParser.Load(filePath); err != nil {
			tools.WriteLogErrorf("try load config file %s  ,error %s \n", filePath, err.Error())
			return err
		}
	}
	err := iniParser.GetFileHandle().MapTo(v)
	if err != nil {
		tools.WriteLogErrorf("MapTo config file %s  ,error %s \n", filePath, err.Error())
	}
	return nil
}
...
func initLogger(logConfig LogConfig) *zap.Logger {
	...
	if logConfig.SocketLoggerEnable {
		...
		if err != nil {
			tools.WriteLogErrorf(logConfig.SocketType, logConfig.SocketIP+":"+logConfig.SocketPort, err.Error())
		} else {
...
/*
func initLogger_bak(lp string, lv string, isDebug bool) {
	var js string
	if isDebug {
		js = fmt.Sprintf(`{
      "level": "%s",
